 ...
 * A sintaxe de uma linguagem de programação são as regras que definem como você
 * deve escrever o código para que o compilador possa entendê-lo. A sintaxe de
 * Java é baseada em C e C++, clara, fortemente tipada, mas com algumas
 * simplificações.
 * Quando consideramos um programa Java, ele pode ser definido como uma coleção
 * de objetos que se comunicam através da invocação dos métodos uns dos outros.
 * Abaixo estão os principais elementos:
 * • Classes e objetos: toda aplicação Java é baseada em classe. O código é
 * organizado dentro de classes e os objetos são instâncias dessas classes;
 * class Pessoa {
 * String nome;
 * }
 * • Métodos: blocos de código que definem comportamentos. São declarados dentro
 * de classes;
 * • void falar() {
 * • System.out.println(“Olá!”);
 * • }
 * • Tipos de dados: primitivos (int, float, char, boolean etc.) e de Referência
 * (string, arrays, objetos personalizados);
 * • Variáveis: armazenam valores e podem ser locais, de instância ou estáticas;
 * int idade = 30;
 * • Operadores: matemáticos (+, -, *, /), relacionais (==,!=, >, <), lógicos
 * (&&, ||,!);
 * • Estruturas de controle: condicionais (if, else, switch) e laços (for,
 * while, do-while);
 * • Modificadores de acesso: controlam a visibilidade de classes, métodos e
 ...
 * +// System.out.print e System.out.printf são usados para exibir saída
 ...
 * +// sem o break, o switch continua executando os próximos cases
 * (fall-through)
 * +// útil para agrupar múltiplos cases que compartilham o mesmo bloco de
 * código
 * + --- IGNORE ---
